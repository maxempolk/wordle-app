...
export function useGameLogic(
  ...
  const stateOfGame = ref<gameState>('in-game')
  ...
  const enterWord = async () => {
    ...
    if(wordState.every((item) => item === 'placed')) stateOfGame.value = 'win'
    else if(words.value.length == 5) stateOfGame.value = 'loss'

    console.log( wordState.every((item) => item === 'placed'), words.value.length )
  ...
  return { enterWord, popLetter, stateOfGame }
}
