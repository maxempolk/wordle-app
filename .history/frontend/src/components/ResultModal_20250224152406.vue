    ...
    <div class="relative rounded-md p-4 w-128 bg-[#3A3A3C] text-white">
      ...
      <div class="flex items-center gap-5 mt-6 h-32">
        <div class="flex w-24 gap-1 ml-6 flex-col">
          <div class="flex gap-1" v-for="word, index in result">
            <div class="size-4" :class="stateToColor(wordleAlgh(word, useRealWordStore().word as string)[index])" v-for="char in word">{{ char }}</div>
          </div>
        </div>
