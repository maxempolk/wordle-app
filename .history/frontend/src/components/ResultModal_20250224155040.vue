...
const allLetterColors = computed( () => {
  const realWord = useRealWordStore().word as string
  const colors: State[][] = [];
  for(const word of props.result){
    colors.push([])
    const wordState = wordleAlgh(word, realWord)

    for(const state of wordState) colors[colors.length-1].push(state)
  }
...
<template>
  <div class="flex w-screen h-screen fixed z-[999] justify-center items-center bg-[#00000066]">
    <div class="relative rounded-md p-4 w-128 bg-[#121213] text-white">
      ...
      <router-link to="/" class="absolute top-4 right-4 size-8 rounded-full flex justify-center items-center bg-[#00000020] hover:bg-[#00000060] focus:bg-[#00000080] cursor-pointer duration-300">
