...
class WordManager:
    def __init__(self, words_file="app/data/words.txt", history_file="app/data/word_history.txt"):
...
# хранить последнюю тысячу слов
def is_word_real(target_word: str):
    with open('app/data/words.txt', 'r', encoding='utf-8') as file:
        for word in file:
            word = word[:-1]
            if word == target_word:
                return True
...
print( is_word_real('писюн') )
print( is_word_real('топка') )
print( is_word_real('толпа') )