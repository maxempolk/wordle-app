...
import { paintWordle } from './utils/wordle'
    ...
    if (e.key == 'Enter' && currentWord.value.length == 5) {
      slotsForCurrentWord().forEach((el: number) => {
        if (letters.value !== null) {
          console.log(store.word, currentWord.value)
          const colors = paintWordle(store.word, currentWord.value)
          letters.value[el].completeAnimation(colors[el])
        }
      })
      completeWord()
