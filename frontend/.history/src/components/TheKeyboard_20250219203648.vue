...
<template>
  <div class="flex gap-1 sm:gap-2 flex-col items-center">
    ...
    <div class="flex gap-1 sm:gap-2">
      <TheKeyboardLetter :model-value="'Enter'" @click="emit('enterWord')" class="py-2" />
      <TheKeyboardLetter
        v-for="char in alph3"
        :key="'letter-' + char"
        :model-value="char"
        @click="keyPress(char)"
      />
      <TheKeyboardLetter :model-value="'<=='" @click="userWordStore.popLetter()" />
    </div>
  </div>
</template>
