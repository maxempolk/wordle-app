...
const enterWord = () => {
  const colors = wordleAlgh(realWordStore.word, usrWordStore.word)
...
<template>
  <div class="flex w-screen h-screen justify-between items-center flex-col py-5 sm:py-16">
    <div class="grid gap-2 grid-cols-5 w-fit">
      <TheLetter v-for="i in 30" :key="i" :model-value="rawData[i - 1]" ref="letter" />
    </div>
    <theKeyboard @enterWord="enterWord" />
  </div>
</template>
